...
function AppEntity(conf, parentData) {
	...
	var watchers = new watchit.WatchIt(restartAppFromWatchIt, displayWatchEvent, conf.folder)
	var watchCopy = new watchcopy.WatchCopy(conf.id, restartAppFromWatchCopyTrigger)
	...
	watchCopy.init(conf.watchCopy, watchCopyInitResult) // init filesystem copy watcher
	...
	*/
	...
	function update(o) { // update this app's configuration
		...
		updateWatcherCount()
		watchCopy.updateCopyObject(conf.watchCopy = o.watchCopy, updateRestartCheck)
		updateRestartCheck(null, restart)

		function watchCopyUppdateResult(err, count, restartFlag) {
			if (!err) {
				watchCopyCount = count
				updateWatcherCount()
				updateRestartCheck(null, restartFlag ? 'file watchers' : null)
			} else self.emit('error', err)
		}

		function updateRestartCheck(err, restartReason) {
			if (!err) {
				if (restartReason) restart = restartReason
				if (!--cbCount) {
					if (restart) log('Restarting ' + conf.name + ' on update due to ' + restart)
...
// TODO
	...
	function watchCopyInitResult(err, count) { // on construction, watch copy init has concluded
		if (!err) {

			// update dashboard watch count
			watchCopyCount = count
			updateWatcherCount()

			if (!parentData.state) appLink.afterThat(conf.state) // ensure app in initial state

			// request url and port information
	...
	function restartAppFromWatchCopyTrigger(err, watchCountDelta) {
		watchCopyCount += watchCountDelta
		restartAppFromWatching(err)
	}

	function restartAppFromWatchIt(err, watchItCount) {
		watchCount = watchItCount
		restartAppFromWatching(err)
	}

	// restart due to file change or copy change
	function restartAppFromWatching(err) {
		updateWatcherCount()
	...
	*/
	...
	function displayWatchEvent(event, filename) {
		log(conf.id, event, filename)
	}
