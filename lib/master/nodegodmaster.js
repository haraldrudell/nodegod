...
function run(opts) {
	var logger = new rotatedlogger.RotatedLog()
	log = logger.log
	...
	function masterResult(isMaster) {
		if (isMaster === true) { // we need to launch the Web ui
			logger.configure({file: true, streamOpts: {logFile: opts.appIndentifier}}) // start writing to the logFile, not only console
			log(processName, 'is master: launching ui', launchTime)
			processfinder.setResetUi(uimanager.getUiRelauncher())
			uimanager.launchUi({processName: processName, launchArray: opts.launchArray, rlog: logger})
